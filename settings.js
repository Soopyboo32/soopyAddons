...
function guiRenderFunction(mouseX, mouseY, partialTicks) {
    ...
    Renderer.scale(2, 2)
    new Text(
        "&6Soopy&7Addons", (menuX + menuWidth / 2 - Renderer.getStringWidth("&6Soopy&7Addons", true) / 1) / 2, (menuY + 5) / 2
    ...
    if (selectedModule == null) {
        ...
        Object.keys(settings.settingsData).forEach((settingModuleKey) => {
            ...
            new Text(
                ...
                (moduleX + moduleWidth / 2 - (Renderer.getStringWidth(settingsModuleData.name, true) * nameScale) / 2) / nameScale, (moduleY + scrollView + 5) / nameScale
            ...
            settingsModuleData.description.forEach((line) => {
                ...
                new Text(
                    line, (moduleX + moduleWidth / 2 - ((Renderer.getStringWidth(line, true) / 2) * 0.75)) / 0.75, (moduleY + scrollView + moduleHeight - 10 - (totalLines * 7) + (lineId * 7)) / 0.75
                ).setColor(Renderer.color(255, 255, 255, (menuOff * 255) * (1 - animOffProg))).draw();
                Renderer.scale(1, 1)
                //}
        ...
        new Text(
            "&7Back", menuX + (40 / 2) - (Renderer.getStringWidth("Back", true) / 2), menuY + 7.5
        ).setColor(Renderer.color(255, 255, 255, menuOff * 255)).draw();
