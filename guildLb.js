...
function guiRenderFunction(mouseX, mouseY, partialTicks) {
    ...
    if((currGuildData.data?.loadedAmount || 0) !== (currGuildData.data?.guildMembers || 0)){
        ...
        drawStringCentered((currGuildData.data?.loadedAmount || 0) + "/" + (currGuildData.data?.guildMembers || 0) + " eta: " + Math.max(Math.ceil(((currGuildData.data?.loadedEta || 0)-new Date().getTime())/1000),-1000000000000000) + "s", topBarX+(topBarWidth/2), topBarY+topBarHeight/2, 1.5)
...
register("tick",()=>{
    ...
    if((currGuildName !== "" && (currGuildData === undefined || currGuildData.data === undefined) || notLoaded) && new Date().getTime()-lastLoadedData > 5000 && !loadingData){
        loadData()
    }
...
function loadData(){
    ...
    lastLoadedData = new Date().getTime()
